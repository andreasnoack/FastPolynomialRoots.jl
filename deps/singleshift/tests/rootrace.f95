...
program main 
  ...
  open (unit=33, file=path//"fig_zgeev_times.txt", status='unknown', position="append")
  open (unit=43, file=path//"fig_zgeev_errors.txt", status='unknown', position="append")

  ! more runs for small degrees
  ! => accurate timing
  num_trials(1) = 2**(14)
  num_trials(2) = 2**(14)
  num_trials(3) = 2**(14)
  ...
  do ll=1,17
     ...
     call normalpoly(n*mm,rcoeffs,icoeffs)
     
     do ii=1,mm
        icoeffs(n+(ii-1)*n)=0d0
        ! for random coefficients is balancing not necessary
 	!call balance(n,rcoeffs(str:stp),icoeffs((ii-1)*n+1:(ii)*n),nnew,rnew((ii-1)*n+1:(ii)*n),inew((ii-1)*n+1:(ii)*n), alpha(ii))
	...
	call zamvw2(n,Q,D,C,B2,reigs,ieigs,its,flag,n-1,0)
        ...
        call RESCHECK(0,N,0,newtnum,POLY((ii-1)*n+1:ii*n),COEFFS,ROOTS,ALLROOTS,RES)
